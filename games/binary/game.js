(() => {
  ...
  let rows = [], score = 0, cleared = 0, level = 1, timer = null, paused = false, active = false;
  ...
  function newChallenge() {
    ...
    const mode = Math.random() < .5 ? 'toDecimal' : 'toBinary';
  ...
  function start() {
    rows = []; score = 0; cleared = 0; level = 1; paused = false; active = true;
    startPanel.classList.add('hidden'); pauseBtn.disabled = false; endBtn.disabled = false; pauseBtn.textContent = 'Pause';
  ...
  function gameOver() {
    ...
    document.querySelector('#start').textContent = 'Run again'; pauseBtn.disabled = true; endBtn.disabled = true;
  }
